...
The apiserver is stateless, and handles leader election for the other master components. The controller-manager and scheduler use leader election to ensure only one of each is active at a time. This means the additional master components are ready to take over in the case of failure.
